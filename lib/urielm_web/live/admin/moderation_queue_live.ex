defmodule UrielmWeb.Admin.ModerationQueueLive do
  ...
  def render(assigns) do
    ...
    >
      <div id="admin-moderation-page" class="ui-page-shell max-w-6xl">
        <header id="admin-moderation-header" class="ui-page-header ui-page-heading">
          <h1 class="ui-section-title">Moderation reports</h1>
          <p class="ui-section-copy">
            Review new reports and revisit past decisions without losing their context.
          </p>
        </header>

        <.admin_nav current="moderation" />

        <section
          id="moderation-report-toolbar"
          class="ui-card ui-card-compact h-auto p-2"
          ...
          <div class="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
            ...
            >
              <%= for status <- @statuses do %>
                <button
                  ...
                  class={[
                    "tab min-h-11 shrink-0 px-3 text-sm font-bold transition-colors duration-150 focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-primary",
            ...
            >
              ...
              <.input
                ...
                autocomplete="off"
                phx-debounce="250"
                class="input input-bordered min-h-11 w-full border-base-300 bg-base-100 pl-10 text-sm focus:outline-2 focus:outline-offset-2 focus:outline-primary"
              />
            </.form>
          </div>
        </section>
        ...
        <.empty_state
          :if={@reports_empty?}
          id="reports-empty"
          title={empty_title(@active_status)}
          description={empty_message(@active_status, @search)}
          icon={empty_icon(@active_status)}
          compact
        />
        ...
        <div id="moderation-reports" phx-update="stream" class="space-y-3">
          <article
            ...
            class="ui-card ui-card-compact h-auto"
          >
              ...
              <details :if={report.status == "pending"} class="group mt-3">
                ...
                >
                  <.input
                    ...
                    maxlength="500"
                    class="input input-bordered min-h-11 w-full border-base-300 bg-base-100"
                  />
            ...
            >
              ...
              <p>
                ...
                <% else %>
                  ...
                  {status_action(report.status)} this · {LiveHelpers.format_short(report.resolved_at)}
                  <span :if={present?(report.resolution_notes)}>
                    · “{report.resolution_notes}”
                  </span>
                <% end %>
